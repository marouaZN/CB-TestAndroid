...
class ProductListActivityTest : TestCase() {
    private var decorView: View? = null

    @Rule
    @JvmField
    var activityRule = ActivityScenarioRule(ProductListActivity::class.java)
    ...
    fun testCaseForRecyclerItemView() {
        Espresso.onView(ViewMatchers.withId(R.id.rvProductList))
            .inRoot(
                RootMatchers.withDecorView(
                    Matchers.`is`(decorView)
                )
            ...
            .check(
                matches(
                    withViewAtPosition(
                        1, Matchers.allOf(
                            ViewMatchers.withId(R.id.rvProductList), ViewMatchers.isDisplayed()
                        )
                    )
                )
            )
    ...
    @Before
    fun loadDecorView() {
        activityRule.scenario.onActivity { activity: ProductListActivity ->
